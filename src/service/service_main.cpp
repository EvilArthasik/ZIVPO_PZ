...
[[nodiscard]] bool DenyProcessTerminate(HANDLE process)
{
    ...
    if (!CreateWellKnownSid(WinWorldSid, nullptr, everyoneSid, &everyoneSidSize)) {
        LocalFree(securityDescriptor);
        return false;
    }

    EXPLICIT_ACCESSW denyTerminate = {};
    denyTerminate.grfAccessPermissions = PROCESS_TERMINATE;
    denyTerminate.grfAccessMode = DENY_ACCESS;
    denyTerminate.grfInheritance = NO_INHERITANCE;
    denyTerminate.Trustee.TrusteeForm = TRUSTEE_IS_SID;
    denyTerminate.Trustee.TrusteeType = TRUSTEE_IS_WELL_KNOWN_GROUP;
    denyTerminate.Trustee.ptstrName = reinterpret_cast<LPWSTR>(everyoneSid);

    PACL newDacl = nullptr;
    result = SetEntriesInAclW(1, &denyTerminate, oldDacl, &newDacl);
    if (result == ERROR_SUCCESS) {
...
[[nodiscard]] DWORD FindActiveUserSession()
{
...
[[nodiscard]] bool ConfirmStopInActiveSession()
{
    const DWORD sessionId = FindActiveUserSession();
    if (sessionId == 0) {
        return true;
    }

    DWORD response = IDNO;
    const BOOL sent = WTSSendMessageW(
        WTS_CURRENT_SERVER_HANDLE,
        sessionId,
        const_cast<LPWSTR>(L"Tray Sample Service"),
        static_cast<DWORD>(sizeof(L"Tray Sample Service")),
        const_cast<LPWSTR>(L"Остановить Windows-службу и закрыть все экземпляры приложения?"),
        static_cast<DWORD>(sizeof(L"Остановить Windows-службу и закрыть все экземпляры приложения?")),
        MB_YESNO | MB_ICONQUESTION | MB_TOPMOST,
        kStopConfirmationTimeoutSeconds,
        &response,
        TRUE);

    return sent != FALSE && response == IDYES;
}
...
[[nodiscard]] bool LaunchGuiForSession(DWORD sessionId)
{
    ...
    const std::wstring appPath = directory + L"\\" + trayapp::kAppExecutableName;
    std::wstring commandLine = Quote(appPath) + L" " +
        trayapp::kServiceChildArg + L" " +
        trayapp::kSilentArg;
    ...
    static_cast<void>(DenyProcessTerminate(processInfo.hProcess));
...
void WINAPI ServiceMain(DWORD, LPWSTR*)
{
    ...
    static_cast<void>(DenyProcessTerminate(GetCurrentProcess()));
...
int InstallService()
{
    ...
    const std::wstring servicePath = directory + L"\\" + trayapp::kServiceExecutableName;
...
int UninstallService()
{
    ...
    ServiceHandle service(OpenServiceW(manager.Get(), trayapp::kServiceName, DELETE));
    if (!service) {
        return 1;
    }
...
int RunService()
{
    SERVICE_TABLE_ENTRYW serviceTable[] = {
        { const_cast<LPWSTR>(trayapp::kServiceName), ServiceMain },
        { nullptr, nullptr }
    };
...
extern "C" void TrayStopService(handle_t)
{
